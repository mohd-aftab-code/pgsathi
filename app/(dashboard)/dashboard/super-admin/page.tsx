...
import { ShieldCheck, Users, Building2, MousePointerClick, RefreshCcw, MoreVertical } from "lucide-react";
  ...
  async function handleAction(userId: number, action: string, days?: number) {
    ...
    try {
      const res = await fetch("/api/admin/users", {
        ...
        body: JSON.stringify({ action, userId, days, planId: 1 }) // Hardcoded planId 1 for now (Basic)
      });
      const d = await res.json();
      if (d.success) {
      ...
      <div className="flex flex-wrap items-end justify-between gap-4 border-b border-neutral-200 pb-5">
        <div>
          <h1 className="text-2xl font-extrabold text-neutral-900 flex items-center gap-2">
            <ShieldCheck className="text-blue-600" size={28} /> Super Admin Control
          </h1>
          <p className="text-sm text-neutral-500 mt-1">SaaS Management & Master CRM Dashboard</p>
        </div>
