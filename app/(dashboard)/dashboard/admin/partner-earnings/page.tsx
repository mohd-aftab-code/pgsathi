...
export default async function AdminPartnerEarningsPage({
  ...
  const tab = (label: string, val: string) => (
    <a href={`/dashboard/admin/partner-earnings?status=${val}`}
      className={`h-8 px-3 rounded-xl text-[10px] font-black inline-flex items-center gap-1.5 uppercase tracking-wider transition-colors shadow-sm ${status === val ? "bg-neutral-900 text-white" : "bg-white/60 backdrop-blur-md border border-neutral-200/60 text-neutral-600 hover:bg-white/80 hover:text-neutral-900"}`}>
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-2">
        <div>
          <h1 className="text-2xl font-black text-neutral-900 tracking-tight uppercase">Partner Earnings</h1>
          <p className="text-[10px] font-bold text-neutral-500 uppercase tracking-wider mt-0.5">Har earning ka amount manually set karein, approve aur pay karein. (Koi commission % nahi.)</p>
        </div>
      </div>
      ...
      {earnings.length === 0 ? (
              ...
              {earnings.map((e: any) => (
                <tr key={e.id} className="hover:bg-white/80 transition-colors group">
                  <td className="px-4 py-2">
                    <div className="font-black text-xs uppercase tracking-tight text-neutral-900">{e.partner.user.name}</div>
                    <div className="text-[9px] font-bold text-neutral-400 tracking-wider uppercase mt-0.5">{e.partner.partnerCode}</div>
                  </td>
                  <td className="px-4 py-2 text-[10px] text-neutral-600 max-w-[150px] truncate font-bold uppercase tracking-tight">{e.owner?.name ?? e.listing?.title ?? "—"}</td>
        ...
        <div className="grid grid-cols-1 gap-2 md:hidden">
          {earnings.map((e: any) => (
            <div key={`mob-${e.id}`} className="bg-white/60 backdrop-blur-md border border-neutral-200/60 rounded-2xl p-4 shadow-sm flex flex-col gap-2">
              <div className="flex justify-between items-start gap-2">
                <div className="min-w-0">
                  <div className="font-black text-sm uppercase tracking-tight text-neutral-900 truncate">{e.partner.user.name}</div>
              ...
              <div className="grid grid-cols-3 gap-1 py-1.5 border-y border-neutral-200/60 text-center">
                <div className="truncate px-1">
                  <div className="text-[9px] text-neutral-400 font-bold uppercase tracking-wider">Owner/PG</div>
                  <div className="text-[10px] font-bold text-neutral-700 truncate">{e.owner?.name ?? e.listing?.title ?? "—"}</div>
                </div>
                <div className="border-l border-neutral-200/60 truncate px-1">
                  <div className="text-[9px] text-neutral-400 font-bold uppercase tracking-wider">Plan</div>
                  <div className="text-[10px] font-bold text-neutral-700 truncate">{e.planNameSnapshot ?? "—"}</div>
                </div>
                <div className="border-l border-neutral-200/60 truncate px-1">
                  <div className="text-[9px] text-neutral-400 font-bold uppercase tracking-wider">Date</div>
                  <div className="text-[10px] font-bold text-neutral-500 truncate">{fmtDate(e.createdAt)}</div>
                </div>
              </div>
      ...
      {totalCount > pageSize && (
        <div className="flex items-center justify-between mt-4 px-2">
          <div className="text-[10px] text-neutral-400 font-black uppercase tracking-wider">
            Showing <span className="font-black text-neutral-900">{(page - 1) * pageSize + 1}</span>–<span className="font-black text-neutral-900">{Math.min(page * pageSize, totalCount)}</span> of <span className="font-black text-neutral-900">{totalCount}</span> earnings
          </div>
          <div className="flex gap-1">
            {page > 1 && (
              <a
                href={`?status=${status}&page=${page - 1}`}
                className="flex items-center gap-1 text-[10px] font-black text-neutral-600 hover:text-violet-700 bg-white/60 backdrop-blur-md border border-neutral-200/60 shadow-sm px-2.5 py-1.5 rounded-xl transition-all uppercase tracking-wider"
              >
                Prev
              </a>
            )}
