...
export default async function PartnerSettingsPage() {
  ...
  const settings = await db.partnerSetting.findUnique({
    ...
    select: { notifyInApp: true, notifyEmail: true, notifyWhatsapp: true },
  });
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      <div className="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-5 shadow-sm">
        ...
        <SettingsForm initial={{ notifyInApp: settings?.notifyInApp ?? true, notifyEmail: settings?.notifyEmail ?? true, notifyWhatsapp: settings?.notifyWhatsapp ?? false }} />
      </div>
